...
const getContractAddresses = async (networkId: any) => {
 ...
 * @param {*} networkId 
 */
const getBiconomyForwarderConfig = async (networkId:any) => {
 ...
 */
const buildForwardTxRequest = async ({account, to, gasLimitNum, batchId, batchNonce, data, deadline}:any) => {
    const req = {
        from: account,
        to: to,
 ...
 * @param {*} networkId 
 */
const getDataToSignForEIP712 = async (request:any,networkId:any) => {
    const contractAddresses = await getContractAddresses(networkId);
    const forwarderAddress = contractAddresses.biconomyForwarderAddress;
 ...
 * @param {*} networkId 
 */
const getDomainSeperator = async (networkId:any) => {
    const contractAddresses = await getContractAddresses(networkId);
    const forwarderAddress = contractAddresses.biconomyForwarderAddress;
    let domainData = helperAttributes.biconomyForwarderDomainData;
    domainData.salt = ethers.utils.hexZeroPad((ethers.BigNumber.from(networkId)).toHexString(), 32);
